...
module.exports = async function imagePosing(imgToParse, parseCallback) {
  ...
  if (typeof (imgToParse) === 'object') {
    // POSES are useful for figuring out the COMPOSITION of
    // the image AND whether someone is laying down.
    // posenet
    // const net = await posenet.load();
    /*
      let imgB = await Image.load(imgToParse);
      tensor = tf.cast(tf.node.decodeImage(imgB.resize({
        width:224,
        height:224}).toBuffer("jpg"),3), 'int32');
    ...
    */
    ...
    let predictionPose;
    // THIS IS VERY FRAGILE.  posenet moves rather quickly, it seems,
    // so this may need to be refactored with more ironclad approaches.... the model is so specific.
    try {
      const net = await posenet.load({
        architecture: 'MobileNetV1',
        outputStride: 16,
